...
| *(Opcional)* `15: clicarLinkPromocao(...)` ... | Usuário clica no link externo; sistema pode registrar o clique.                | Método `registrarCliquePromocao` (`ModuloPromocoes`, inf.), `logClique` (BD, inf.).                                                                                                        | (Indireto) Suporta propósito de RF19.    | Chamada async opcional para log (`ModuloPromocoes` -> `IPersistencia`). Ação de redirect na `WebUI`.                                                                                              |
