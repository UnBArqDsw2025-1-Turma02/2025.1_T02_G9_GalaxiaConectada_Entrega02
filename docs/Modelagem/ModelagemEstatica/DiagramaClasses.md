...
| **Aluno**         | Subclasse de Usuário; representa um aluno.   | `cursosRealizados: List<Curso>`<br> `trilhasConcluidas: List<TrilhaEducacional>`<br> `certificados: List<Certificado>` | + `iniciarCurso(c: Curso): Void`<br> + `finalizarTrilha(t: TrilhaEducacional): Void` | - TrilhaEducacional ( *:*, associação, “estuda”)<br> - Curso ( *:*, associação, “realiza”) |
| **Instrutor**     | Subclasse de Usuário; representa um instrutor. | `trilhasCriadas: List<TrilhaEducacional>`<br> `materiaisCriados: List<Conteudo>` | + `criarTrilha(t: TrilhaEducacional): Void`<br> + `avaliarAluno(a: Aluno, t: TrilhaEducacional): Void` | - TrilhaEducacional ( *:1, associação, “ensina”)<br> - Conteudo ( *:*, associação, “cria”) |
| **Administrador**  | Subclasse de Usuário; representa um administrador do sistema. | `permissoes: List<String>` | + `gerenciarUsuarios(): Void`<br> + `modificarSistema(): Void` | - Usuario ( *:*, associação, “gerencia”) |
| **Professor_Voluntario**          | subclasse de Usuário. Representa instrutores responsáveis pela aplicação de artigos e manutenção dos conteúdos gerais. | - `id: Int` (private)<br>- `nome: String` (private)<br>- `areaEspecialidade: String` (private)                                                               | + `criarTrilha(t: TrilhaEducacional): Void`<br>+ `avaliarConteudo(c: Conteudo): Void`                                              | TrilhaEducacional (1:*, associação, “ministra”)                                                                                                                  |
...
| **Topico**             | Discussões iniciadas dentro de um subfórum. | - `id: Int` (private)<br>- `titulo: String` (private)<br>- `dataCriacao: DateTime` (private)                                                                | + `adicionarPostagem(p: Postagem): Void`<br>+ `fechar(): Void`                                                                     | Subforum ( *:1, associação, “ pertence a”)<br>Postagem ( *:*, associação, “tem”)<br>Usuario ( *:1, associação, “cria”) |
| **Postagem**           | Representa uma contribuição feita pelos usuários em um tópico. | - `id: Int` (private)<br>- `texto: String` (private)<br>- `dataPostagem: DateTime` (private)                                                                 | + `adicionarComentario(c: Comentario): Void`                                                                                      | Topico ( *:1, associação, “pertence a”)<br>Usuario ( *:1, associação, “fez”)<br>Comentario ( 1:*, associação, “comenta”) |
